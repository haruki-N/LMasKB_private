...
"""
    11/17時点でmain.py(通常BART)をmeta learning化できるようにきちんと整えた。
    それをベースにDecoderConstrainedBART用のプログラムに書き換え
"""
...
def main():
    ...
    one2one_data_loader = prepare_dataloader(parent_labels, one2one_label, input_pad_length=24, label_pad_length=8,
                                             batch_size=batch_size, tokenizer=tokenizer,
                                             decoder_tokenizer=decoder_tokenizer, shuffle=True)

    # prepare train, valid, test dataloader
    train_inputs = one2one_sentences + train_parent
    train_label = one2one_label + train_child_label
    train_loader = prepare_dataloader(train_inputs, train_label, input_pad_length=24, label_pad_length=8, shuffle=True,
                                      batch_size=batch_size, tokenizer=tokenizer, decoder_tokenizer=decoder_tokenizer)
    ...
    valid_loader = prepare_dataloader(valid_parent, valid_child_label, input_pad_length=24, label_pad_length=8, shuffle=True,
                                      tokenizer=tokenizer, batch_size=batch_size, decoder_tokenizer=decoder_tokenizer)
    test_loader = prepare_dataloader(test_parent, test_child_label, input_pad_length=24, label_pad_length=8, shuffle=True,
                                     tokenizer=tokenizer, batch_size=batch_size, decoder_tokenizer=decoder_tokenizer)
    ...
    if fewshots_batch == 0:
        fewshots_batch = 8
    fewshots_data_loader = prepare_dataloader(fewshots_sentences, fewshots_labels, input_pad_length=24,
                                              label_pad_length=8, batch_size=fewshots_batch, tokenizer=tokenizer,
                                              decoder_tokenizer=decoder_tokenizer, shuffle=True)
    ...
    mix_data_loader = prepare_dataloader(mix_sentences, mix_label, input_pad_length=24, label_pad_length=8,
                                         batch_size=batch_size, tokenizer=tokenizer,
                                         decoder_tokenizer=decoder_tokenizer, shuffle=True)
    ...
    one2n_data_loader = prepare_dataloader(org_masked_sentences, child_pairs_label, input_pad_length=24,
                                           label_pad_length=8, batch_size=batch_size, tokenizer=tokenizer,
                                           decoder_tokenizer=decoder_tokenizer, shuffle=True)

    # all
    all_sent = one2one_sentences + org_masked_sentences
    all_label = one2one_label + child_pairs_label
    all_data_loader = prepare_dataloader(all_sent, all_label, input_pad_length=24, label_pad_length=8,
                                         batch_size=batch_size, tokenizer=tokenizer,
                                         decoder_tokenizer=decoder_tokenizer, shuffle=True)
    ...
    one2two_data_loader = prepare_dataloader(one2two_sent, one2two_label, input_pad_length=24, label_pad_length=8,
                                             batch_size=batch_size, tokenizer=tokenizer,
                                             decoder_tokenizer=decoder_tokenizer, shuffle=True)
    ...
    one2three_data_loader = prepare_dataloader(one2three_sent, one2three_label, input_pad_length=24, label_pad_length=8,
                                               batch_size=batch_size, tokenizer=tokenizer,
                                               decoder_tokenizer=decoder_tokenizer, shuffle=True)
    ...
    one2four_data_loader = prepare_dataloader(one2four_sent, one2four_label, input_pad_length=24, label_pad_length=8,
                                              batch_size=batch_size, tokenizer=tokenizer,
                                              decoder_tokenizer=decoder_tokenizer, shuffle=True)
    ...
    for _epoch in tqdm.tqdm(range(epoch_num)):
        ...
        model.eval()
        acc_train.append(model.get_acc(train_loader, device))
        acc_valid.append(model.get_acc(valid_loader, device))
        acc_test.append(model.get_acc(test_loader, device))
        # acc_list_one2one.append(model.get_acc(one2one_data_loader, device))
    ...
    if second_task:
        print(f'Second task will be started. {second_task}')
        # --------- freeze encoder params ---------
        if freeze_encoder:
            for name, param in model.named_parameters():
                if 'decoder' not in name:
                    param.requires_grad = False
                if name == 'bart.lm_head.weight':
        ...
        for _epoch in tqdm.tqdm(range(epoch_num)):
            ...
            train_loss.append(running_loss / len(batch))
            model.eval()
            acc_list_one2one.append(model.get_acc(mix_data_loader, device))
            acc_list_1toN.append(model.get_acc(one2n_data_loader, device))
            acc_list_1to2.append(model.get_acc(one2two_data_loader, device))
            acc_list_1to3.append(model.get_acc(one2three_data_loader, device))
            acc_list_1to4.append(model.get_acc(one2four_data_loader, device))
    ...
    accs = [acc_train, acc_valid, acc_test]

    labels = ['seen relations(1to1 + few 1toN)', '1toN Relations', '1to2', '1to3', '1to4']
    labels = ['train(1to1 + 30%1toN)', 'valid(30%1toN)', 'test(40%1toN)']
    if second_task:
        title = f'../1toN_result/constBART_id_one2one_then_{second_task}_numbering_{ratio}%shot_result_{children_num*3}.png'
        if freeze_encoder:
            title = f'../1toN_result/constBART_id_one2one_then_EncFreezed_{second_task}_numbering_{ratio}%shot_result_{children_num*3}.png'
    else:
        title = f'../1toN_result/constBART_id_train_valid_test_only_numbering_result_default_{children_num*3}.png'

    print(title)

    show_acc_loss(numerical_settings, accs, [train_loss, valid_loss_list], title, labels=labels)
