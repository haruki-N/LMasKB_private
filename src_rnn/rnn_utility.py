...
def get_args():
    ...
    parser.add_argument('--relation', type=int, default=0, choices=[0, 2, 3, 4], help="number of relationship(1-to-N).0=mix")
    parser.add_argument('--children', type=int, default=360, help="number of children")
    ...
    parser.add_argument('--mix_ratio', type=int, default=10, choices=[10, 30, 50, 70])
...
def get_last_dimention(ndim_list: list):
    if isinstance(ndim_list[0], list):
        return get_last_dimention(ndim_list[0])
    else:
        return len(ndim_list)
...
def identify_outliers(y: list, mode='iqr'):
...
def decompose_relation(parents: list, children: list):
    ...
    for parent, child_pair in zip(parents, children):
        ...
        sentence = f'{parent} has a child named <mask>'
